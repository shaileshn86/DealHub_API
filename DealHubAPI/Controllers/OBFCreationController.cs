...
namespace DealHubAPI.Controllers
{
    ...
    public class OBFCreationController : BaseApiController
    {
        ...
        protected void getactualobfparams(ObfCreationParameters model)
        {
            ...
            model._total_revenue = model._total_revenue.ToString().Length <= 4 ? 0 : Convert.ToDecimal(model._total_revenue.ToString().Substring(0, (model._total_revenue.ToString().Length - 4)));
            model._payment_terms = model._payment_terms.ToString().Length <= 4 ? 0 : Convert.ToInt32(model._payment_terms.ToString().Substring(0, (model._payment_terms.ToString().Length - 4)));
            model._vertical_id = model._vertical_id.ToString().Length <= 4 ? 0 : Convert.ToInt32(model._vertical_id.ToString().Substring(0, (model._vertical_id.ToString().Length - 4)));
        }
    }

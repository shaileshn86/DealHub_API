...
{
    public class MstSolutionCategory:BaseDAL
    {
        public static string GetMstSolutionCategory(Mstcommonparameters model)
        {
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("sp_get_mst_solutioncategory", conn);
        ...
        public static List<commanmessges> UpdateMstSolutionCategory(MstSolutionCategoryParameters model)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    MySqlCommand cmd = new MySqlCommand("sp_update_mst_solutioncategory", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("_solutioncategory_Id", MySqlDbType.UInt32).Value = model._solutioncategory_Id;
                    cmd.Parameters.Add("_solutioncategory_name", MySqlDbType.String).Value = model._solutioncategory_name;

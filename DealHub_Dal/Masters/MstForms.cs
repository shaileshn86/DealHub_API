    ...
    public class MstForms:BaseDAL
    {
        public static string GetMstForms(Mstcommonparameters model)
        {
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("sp_get_mst_forms", conn);
        ...
        public static List<commanmessges> Update_Mst_Forms(MstFormParameters model)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    MySqlCommand cmd = new MySqlCommand("sp_update_mst_forms", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("_id", MySqlDbType.UInt32).Value = model._id;
                    cmd.Parameters.Add("_form_name", MySqlDbType.String).Value = model._form_name;
                    cmd.Parameters.Add("_url", MySqlDbType.String).Value = model._url;

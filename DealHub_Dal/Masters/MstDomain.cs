...
{
    public class MstDomain :BaseDAL
    {
        public static string GetMstDomains(Mstcommonparameters model)
        {
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("sp_get_mst_domains", conn);
        ...
        public static List<commanmessges> Update_Mst_Domains(MstDomainParameters model)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    MySqlCommand cmd = new MySqlCommand("sp_update_mst_domains", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("_domain_id", MySqlDbType.UInt32).Value = model._domain_id;
                    cmd.Parameters.Add("_domain_code", MySqlDbType.String).Value = model._domain_code;
                    cmd.Parameters.Add("_domain_name", MySqlDbType.String).Value = model._domain_name;

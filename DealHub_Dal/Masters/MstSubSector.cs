...
{
    public class MstSubSector:BaseDAL
    {
        public static string GetMstSubsector(Mstcommonparameters model)
        {
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    ...
                    MySqlCommand cmd = new MySqlCommand("sp_get_mst_subsector", conn);
        ...
        public static List<commanmessges> Update_Mst_Subsector(MstSubSectorsParameters model)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connectionString))
                {
                    MySqlCommand cmd = new MySqlCommand("sp_update_mst_subsector", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("_SubSector_Id", MySqlDbType.UInt32).Value = model._SubSector_Id;
                    cmd.Parameters.Add("_SubSector_Name", MySqlDbType.String).Value = model._SubSector_Name;
                    cmd.Parameters.Add("_Sector_Id", MySqlDbType.String).Value = model._Sector_Id;
